...
class CanaryTRTLLM(object):
    ...
    def __init__(self,
        ...
        self.preprocessor = AudioToMelSpectrogramPreprocessor(sample_rate=16000,
                                               normalize="per_feature",
                                               window_size=0.025,
                                               window_stride=0.01,
                                               window="hann",features=128,n_fft=512,log=True,
                                               frame_splicing=1,dither=1e-5,pad_to=0,pad_value=0.0,
                                               preemph=preemp
                                               ).to(self.device)
                                               
        #self.tokenizer = CanaryTokenizer(engine_dir)
        self.model = EncDecMultiTaskModel.restore_from(nemo_dir, map_location=map_location)
        torch.cuda.empty_cache()
        self.tokenizer = self.model.tokenizer
        #self.model.tokenizer.save_to("/inference/examples/canary/canary_model/tokenizer_dir")
        self.tokenizer2 = CanaryTokenizer(engine_dir)
    ...
    def process_batch(
        ...
        prompt_formatter = Canary2PromptFormatter(tokenizer=self.model.tokenizer)
        turns = [dict(
            role="user",
            slots={
                "decodercontext": "", "source_lang": "<|mr|>", "target_lang": "<|en|>",
                "emotion": "<|emo:undefined|>", "pnc": "<|pnc|>", "itn": "<|noitn|>",
                "diarize": "<|nodiarize|>", "timestamp": "<|notimestamp|>", "prompt_language": "spl_tokens"
            }
        )]
        encoded_dict = prompt_formatter.encode_dialog(turns)
        decoder_input_ids = encoded_dict['input_ids'].to(self.device).unsqueeze(0)
        decoder_input_ids = decoder_input_ids[:, :3]  # Only first 3 tokens: <|startoftext|><|startoftranscript|><|lid|>
        #decoder_len = torch.tensor([decoder_input_ids.shape[1]], device=self.device)
        decoder_input_ids = decoder_input_ids.expand(batch_size, -1).contiguous()
            # decoder_len should be a tensor of shape (batch_size,) with each value = decoder_input.shape[1]
        decoder_len = torch.full((batch_size,), decoder_input_ids.shape[1], device=decoder_input_ids.device, dtype=torch.long)
        stream = torch.cuda.current_stream('cuda')

        if max_new_tokens is None:
            max_new_tokens = self.max_seq_len

        # Convert list of tensors to batched tensor for NeMo preprocessor
        if isinstance(audio, list):
            # Pad sequences to the same length
            audio_tensor = torch.nn.utils.rnn.pad_sequence(
                audio, batch_first=True, padding_value=0.0).to(self.device)
            audio_len_tensor = torch.tensor(audio_input_lengths, dtype=torch.int64, device=self.device)
        else:
            audio_tensor = audio.to(self.device)
            audio_len_tensor = torch.tensor(audio_input_lengths, device=self.device)
        #np.save("audio_tensor.npy", audio_tensor.cpu().numpy())
        #mel, mel_input_lengths = self.preprocessor.get_features(
        #    input_signal=audio_tensor, length=audio_len_tensor)
        #audio_tensor=torch.from_numpy(np.load("mel-2.npy")).to(self.device)
        mel, mel_input_lengths = self.preprocessor(
            input_signal=audio_tensor,
            length=audio_len_tensor,
        )
        #mel=torch.from_numpy(np.load("mel-2.npy")).to(self.device)
        encoder_output, encoder_output_lengths = self.encoder.infer(
            mel, mel_input_lengths, stream)
        #encoder_output=torch.from_numpy(np.load("encoder_output_nemo.npy")).to(self.device)
